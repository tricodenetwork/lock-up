    ...
    <div className='p-6 bg-white rounded-3xl   overflow-hidden border-none shadow-none'>
          ...
          <div className='overflow-x-auto '>
            <table className='w-full'>
              ...
              <tbody>
                {isIntermediary ?transactions.reverse().map((item:Transaction, i) => (
                  <tr key={i.toString()} className='border-b'>
                    <td className='py-3 text-xs '>{new Date(parseInt((item.finished ?? item.started) as unknown as string)).toLocaleString()}</td>
                    <td className='py-3 text-xs '>_____</td>
                    <td className='py-3 text-xs text-appBlue '>{item.flag} <span className="text-appBlack  text-[8px]">{item.currency}</span> {item.amount_in_fiat}</td>
                    <td className='py-3 text-xs text-appBlue '>{item.flag} <span className="text-appBlack  text-[8px]">{item.currency}</span> {item.amount_in_fiat}</td>
                    <td className='py-3 '>
                      <p
                  ...
                  <tr key={i.toString()} className='border-b'>
                    <td className='py-3 text-xs '>{new Date(parseInt(item.created as unknown as string)).toLocaleString()}</td>
                    <td className='py-3 text-xs '>_____</td>
                    <td className='py-3 text-xs text-appBlue '>{item.in.flag} <span className="text-appBlack  text-[8px]">{item.in.symbol}</span> {item.in.amount_in_fiat}</td>
                    <td className='py-3 text-xs text-appBlue '>{item.out.flag} <span className="text-appBlack  text-[8px]">{item.out.symbol}</span> {item.out.amount_in_fiat}</td>
                    <td className='py-3 '>
                      <p
                        className={`px-2 py-2 rounded-full font-semibold w-[80px] text-center  text-xs ${
                      ...
                      >
                        {item.in.received && item.out.received ? "Completed" : "Ongoing"}
                      </p>
                    </td>
                    <td className='py-3 flex justify-end'>
                      <button>
